...
# Step 1: Load the data for Case 3 and Case 4 (adjust paths as needed)
case3 <- read_excel("Mcase3 process.xls")
case4 <- read_excel("Mcase4 process.xls")

# Step 2: Extract the relevant parameters for Case 3 and Case 4
# Columns: 15(omega_v), 16(alpha), 17 (Beta_s), 18 (Beta_a), 19 (Xi_v), 20 (Epsilon_v), 25 (Delta), 34 (p_s), 35 (p_a), 27 (R0)
params_case3 <- case3[, c(15, 16, 17, 18, 19, 20, 25, 34, 35, 27)]  # Include R0 (column 27)
params_case4 <- case4[, c(15, 16, 17, 18, 19, 20, 25, 34, 35, 27)]
...
nrow_case4 <- nrow(params_case4)

# Determine the smaller size
min_rows <- min(nrow_case3, nrow_case4)

# Truncate both data frames to the size of the smaller one
params_case3_truncated <- params_case3[1:min_rows, ]
params_case4_truncated <- params_case4[1:min_rows, ]

# Step 4: Calculate the parameter differences between Case 3 and Case 4 (after truncating)
diff_params <- params_case4_truncated - params_case3_truncated
...
# Apply the bootstrap resampling for Case 3 to 4
bootstrap_case3_to_4 <- bootstrap_resampling(diff_params)

# Step 5: Perform Kernel Density Estimation (KDE) on bootstrapped samples
kde_results <- lapply(1:ncol(diff_params), function(i) kde(bootstrap_case3_to_4$t[, i]))
...
# Observed values of parameter differences (this is the actual difference between Case 3 and Case 4)
...
param_indices_H2 <- c(7,8,3)

# Immune Selection: Parameters alpha, omega_v, delta
param_indices_H4 <- c(1,2,3)
...
likelihood_H1 <- joint_likelihood_estimation(bootstrap_case3_to_4$t, observed_diffs, param_indices_H1)

# Likelihood for Hypothesis 2: short-sighted evolution
likelihood_H2 <- joint_likelihood_estimation(bootstrap_case3_to_4$t, observed_diffs, param_indices_H2)

# Likelihood for Hypothesis 4: Immune Selection
likelihood_H4 <- joint_likelihood_estimation(bootstrap_case3_to_4$t, observed_diffs, param_indices_H4)
...
prior_H2 <- 1/3  # short-sighted evolution
prior_H4 <- 1/3  #Immune selection

# Step 13: Compute Posterior Probabilities Using Bayes' Theorem
total_likelihood <- likelihood_H1 * prior_H1 + likelihood_H2 * prior_H2+likelihood_H4 * prior_H4
